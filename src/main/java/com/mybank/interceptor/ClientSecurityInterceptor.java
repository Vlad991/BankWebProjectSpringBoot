...
import static com.mybank.controller.websocket.ClientWebSocketController.CLIENT_LOGIN;
...
public class ClientSecurityInterceptor implements HandshakeInterceptor {
    ...
    public ClientSecurityInterceptor(String serviceName) {
    ...
    public boolean beforeHandshake(ServerHttpRequest request,
        ...
        if (!isClientRole(token)) {
            response.setStatusCode(HttpStatus.FORBIDDEN);
            return false;
        }

        String login = getClientLogin(token);
        if (login == null) {
            response.setStatusCode(HttpStatus.BAD_REQUEST);
            return false;
        }
        attributes.put(CLIENT_LOGIN, login);
        return true;
    }
    ...
    private boolean isClientRole(String token) throws IOException {
        Map<String, Object> json = getBodyTokenAsJson(token);
    ...
    private String getClientLogin(String token) throws IOException {
        Map<String, Object> json = getBodyTokenAsJson(token);
