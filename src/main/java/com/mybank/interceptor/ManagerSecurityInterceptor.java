...
import static com.mybank.controller.websocket.ManagerWebSocketController.MANAGER_LOGIN;

public class ManagerSecurityInterceptor implements HandshakeInterceptor {

    private ObjectMapper mapper = new ObjectMapper();

    private String serviceName;

    public ManagerSecurityInterceptor(String serviceName) {
    ...
    public boolean beforeHandshake(ServerHttpRequest request,
        ...
        if (!isManagerRole(token)) {
            response.setStatusCode(HttpStatus.FORBIDDEN);
            return false;
        }

        String login = getManagerLogin(token);
        if (login == null) {
            response.setStatusCode(HttpStatus.BAD_REQUEST);
            return false;
        }
        attributes.put(MANAGER_LOGIN, login);
        return true;
    }
    ...
    private boolean isManagerRole(String token) throws IOException {
        Map<String, Object> json = getBodyTokenAsJson(token);
    ...
    private String getManagerLogin(String token) throws IOException {
        Map<String, Object> json = getBodyTokenAsJson(token);
